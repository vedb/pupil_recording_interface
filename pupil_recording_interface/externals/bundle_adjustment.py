...
class BundleAdjustment:
    ...
    def _calculate_jacobian_matrix(self, variables, all_observations):
        ...
        rotations, translations, gaze_targets = self._decompose_variables(
            variables
        )
        ...
        if not self._fix_gaze_targets:
            data_targets = [
                -get_jac_trans(translation) for translation in translations
            ]
        ...
        jacobian_matrix = scipy_sparse.csc_matrix(
            (data, (self._row_ind, self._col_ind)),
            shape=(n_residuals, n_variables),
        )
        return jacobian_matrix

    def _least_squares(
        self, initial_guess, all_observations, tol=1e-8, max_nfev=100
    ):
        x_scale = np.ones(self._n_poses_variables)
        if not self._fix_gaze_targets:
            x_scale = (
                np.append(x_scale, np.ones(self._gaze_targets_size) * 500) / 20
            )
    ...
    def _decompose_variables(self, variables):
        ...
        gaze_targets = self._current_values[
            -self._gaze_targets_size :
        ].reshape(-1, 3)
    ...
    def _get_final_output(self, result):
        ...
        (
            ...
            translations,
            final_gaze_targets,
        ) = self._decompose_variables(result.x)
        final_poses = [
