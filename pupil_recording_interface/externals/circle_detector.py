...
from pupil_recording_interface.externals.methods import (
    dist_pts_ellipse,
    normalize,
)
        ...
        if self._flag_check:
            ...
            if len(markers) > 0:
                if len(self._previous_markers) in (0, len(markers)):
                    self._flag_check = True
                    if not self._detection_method.lower() == "vedb":
...
def find_pupil_circle_marker(img, scale):
    ...
    for i in range(len(edges)):
        ...
        for ellipses, boundary in circle_clusters:
            ...
            cv2.ellipse(
                mask_ring, outer_ellipse, color=(0, 0, 0), thickness=-1
            )
            cv2.ellipse(
                mask_ring, inner_ellipse, color=(255, 255, 255), thickness=-1
            )
            mask_ring_mean = np.ma.array(mask_edge, mask=mask_ring).mean()

            mask_outer = np.zeros_like(img_ellipse)
            cv2.ellipse(
                mask_outer, outer_ellipse, color=(255, 255, 255), thickness=-1
            )
            outer_mean = np.ma.array(img_ellipse, mask=mask_outer).mean()
            ring_median = np.ma.median(
                np.ma.array(img_ellipse, mask=mask_ring)
            )
            ...
            if mask_ring_mean >= 128:
                ...
                cv2.ellipse(
                    mask_middle, inner_ellipse, color=(0, 0, 0), thickness=-1
                )
                cv2.ellipse(
                    mask_middle,
                    inner_ellipse,
                ...
                ellipses_list.append(
                    {"ellipses": single_marker, "marker_type": "Ref"}
                )
            ...
            # Check if it is a stop marker
                ...
                ellipses_list.append(
                    {"ellipses": single_marker, "marker_type": "Stop"}
                )
                found_pos.append(ellipse_pos)
                found_size.append(ellipse_size)
    ...
    if first_check:
        ...
        *_, contours, hierarchy = cv2.findContours(
        ...
        if contours is None or hierarchy is None:
            return []
        clusters = get_nested_clusters(
            contours, hierarchy[0], min_ellipses_num
        )
        ...
        for cluster in clusters:
                    ...
                    if len(c) >= 5:
                        ...
                        if first_ellipse:
                            duplicates = [
                                k
                                for k in range(len(found_pos))
                                if LA.norm(e_center - found_pos[k])
                                < found_size[k] + 1
                            ]
                            if len(duplicates) > 0:
                                ellipses[i] = e, 100.0
            ...
            candidate_ellipses = [
                (e[0], e[1], e[2]) for e in candidate_ellipses
            ]
        ...
        for cluster in clusters:
            ...
            for i in cluster:
                ...
                if first_ellipse:
                    ...
                    if img_contrast <= 96:
                        fit_thres = (
                            max(e[1]) * scale / 10 + (256 - img_contrast) / 256
                        )
                    else:
