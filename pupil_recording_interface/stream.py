...
class BaseStream(object):
    ...
    def from_config(cls, config, device=None):
        ...
        if isinstance(config, VideoConfig):
            return VideoStream._from_config(config, device)
        elif isinstance(config, OdometryConfig):
            return OdometryStream._from_config(config, device)
        else:
    ...
    def run(self):
        ...
        for status in self.run_in_thread():
            yield status
...
class VideoStream(BaseStream):
    ...
    def _from_config(cls, config, device=None):
        ...
        return cls(
            device,
            name=config.name,
            pipeline=Pipeline.from_config(config, device),
