...
from pupil_recording_interface import \
    ...
    OdometryReader, VideoReader, OpticalFlowReader
...
class TestGazeReader(object):
    ...
    def test_load_dataset(self, folder):
        ...
        # offline 2d mapper
        ds = GazeReader(
            folder, source=self.gaze_mappers['2d']).load_dataset()
        ...
        # merged 2d/3d gaze
        ds = GazeReader(
            folder, source=self.gaze_mappers).load_dataset()
...
class TestOdometryReader(object):
    ...
    def test_load_dataset(self, folder):
        ...
        with pytest.raises(ValueError):
            OdometryReader(
                folder, source='not_supported').load_dataset()
...
class TestVideoReader(object):
    ...
    def test_read_frames(self, folder):
        ...
        # sub-sampled frame
        interface = VideoReader(folder, subsampling=2.)
    ...
    def test_load_dataset(self, folder):
        """"""
        interface = VideoReader(
            folder, subsampling=8., color_format='gray')
...
class TestOpticalFlowReader(object):
    ...
    def test_load_dataset(self, folder):
        ...
        interface = OpticalFlowReader(folder, subsampling=8.)
